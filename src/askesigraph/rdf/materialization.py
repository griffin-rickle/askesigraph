...
from pyoxigraph import Literal, NamedNode, Quad, QuerySolution, Store
...
class MaterializationEngine:
            ...
            mappings.append(
                {
                    ...
                    "factor": (
                        float(solution["factor"]) if solution.get("factor") else None
                    ),
                    "source_format": (
                        str(solution["sourceFormat"])
                        if solution.get("sourceFormat")
                        else None
                    ),
                }
            )
    ...
    def _materialize_measurement(
        self, vendor_data: QuerySolution, mappings: list[Dict[str, Any]]
    ) -> Iterator[Quad]:
        """Apply mappings to create core ontology triples"""

        # Generate core measurement IRI
        vendor_measurement = vendor_data["measurement"].value
        ...
        for mapping in mappings:
            ...
            target_value = self._apply_transformation(
                source_value, transform_type, mapping
            )

            if target_value is not None:
                yield Quad(core_subject, NamedNode(target_prop), target_value, graph)

    def _apply_transformation(
        self, source_value: Any, transform_type: str, mapping: Dict[str, Any]
    ) -> Literal:
        ...
        if "MultiplyTransform" in transform_type:
            ...
            return Literal(str(result), datatype=NamedNode(f"{Namespace.XSD}decimal"))
            ...
            return Literal(
                dt.isoformat(), datatype=NamedNode(f"{Namespace.XSD}dateTime")
            )
